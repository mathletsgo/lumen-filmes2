...
export function AgeBadge({ code, size = "md", className = "" }: Props) {
  if (!code) return null;
  const style = STYLES[code] ?? { bg: "bg-muted-foreground/40", label: code, ring: "ring-foreground/10" };
  return (
    <span
      title={code === "L" ? "Livre para todos os públicos" : `Não recomendado para menores de ${code} anos`}
