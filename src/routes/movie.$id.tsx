...
import { Download, Heart, Star, ArrowLeft, Share2, Magnet, Users, ArrowDown, Languages, HardDrive, Copy, Check, Play, Clock, Sparkles, Eye } from "lucide-react";
    ...
    return (
      <div className="min-h-screen grid place-items-center text-center px-4">
        <div>
          ...
          <button onClick={() => { router.invalidate(); reset(); }} className="mt-4 px-5 py-2 rounded-full gradient-primary">Tentar novamente</button>
        </div>
      </div>
    );
  },
  notFoundComponent: () => (
    <div className="min-h-screen grid place-items-center text-center px-4">
      <div>
        ...
        <Link to="/" className="inline-block mt-4 px-5 py-2 rounded-full gradient-primary">Voltar</Link>
      </div>
    </div>
  ),
});
  ...
  return (
    <article>
      ...
      <section className="relative h-[80vh] min-h-[560px] w-full overflow-hidden">
        ...
        <Link to="/" className="absolute top-24 left-6 sm:left-12 inline-flex items-center gap-2 px-4 py-2 rounded-full glass hover:bg-foreground/10 transition-colors text-sm">
      ...
      <div className="relative z-10 -mt-64 sm:-mt-80 max-w-screen-xl mx-auto px-4 sm:px-12">
        <div className="grid lg:grid-cols-[280px_1fr] gap-8 lg:gap-12">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}>
            <h1 className="text-4xl sm:text-6xl font-black tracking-tight text-shadow-cinema">{movie.title}</h1>
            <div className="flex flex-wrap items-center gap-3 mt-4 text-sm">
              ...
              {movie.genres.map((g) => (
                <Link key={g} to="/categories" className="px-3 py-1 rounded-full glass text-xs hover:bg-primary/20 transition-colors">
                  {g}
                </Link>
              ))}
            </div>

            <p className="mt-6 text-base sm:text-lg text-foreground/85 max-w-3xl leading-relaxed">{movie.synopsis}</p>

            <div className="flex flex-wrap gap-3 mt-8">
              {/* <a href={movie.downloads[0].magnet} className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full gradient-primary text-primary-foreground font-semibold shadow-glow hover:scale-105 transition-transform">
                <Magnet className="w-5 h-5" /> Magnet Link
              </a>
              <a href={movie.downloads[0].torrent} className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full glass-strong font-semibold hover:bg-foreground/10 transition-colors">
                <Download className="w-5 h-5" /> Baixar Torrent
              </a> */}
              <button disabled className="relative overflow-hidden inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-gradient-to-r from-violet-600 via-fuchsia-600 to-orange-500 text-white font-bold shadow-[0_0_20px_rgba(217,70,239,0.3)] opacity-90 cursor-not-allowed">
                <Sparkles className="w-5 h-5 text-yellow-200 animate-pulse" /> 
                Em breve: Download e Streaming
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 px-6 py-3.5 rounded-full font-semibold transition-all ${
                  fav ? "bg-primary/20 text-primary border border-primary/40" : "glass-strong hover:bg-foreground/10"
                }`}
              >
                <Heart className={`w-5 h-5 ${fav ? "fill-current" : ""}`} />
                {fav ? "Favoritado" : "Favoritar"}
              </button>
              <button className="w-12 h-12 grid place-items-center rounded-full glass-strong hover:bg-foreground/10">
            ...
            {movie.cast.length > 0 && (
              <div className="mt-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                  {movie.cast.map((c) => (
                    <div key={c.name} className="p-4 rounded-2xl glass hover:bg-primary/10 transition-colors">
      ...
      {trailerOpen && movie.trailerKey && (
        ...
        >
          <div className="w-full max-w-5xl aspect-video rounded-2xl overflow-hidden shadow-elevated" onClick={(e) => e.stopPropagation()}>
            <iframe
...
function DownloadRow({ d }: { d: Dl }) {
  ...
  const qualityColor =
    d.quality === "4K" ? "from-accent to-primary" : d.quality === "1080p" ? "from-primary to-accent" : "from-muted-foreground/40 to-muted-foreground/20";
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-4">
        <div className={`px-3 py-1.5 rounded-lg bg-gradient-to-br ${qualityColor} text-primary-foreground text-sm font-bold shadow-glow shrink-0`}>
          {d.quality}
        </div>
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm flex-1 min-w-0">
          <span className="flex items-center gap-1.5 text-muted-foreground"><HardDrive className="w-4 h-4" /> {d.size}</span>
          <span className="flex items-center gap-1.5 text-muted-foreground"><Languages className="w-4 h-4" /> {d.language} · {d.audio}</span>
          <span className="flex items-center gap-1.5 text-emerald-400"><ArrowDown className="w-4 h-4" /> {d.seeders.toLocaleString()} seeders</span>
          <span className="flex items-center gap-1.5 text-orange-400"><Users className="w-4 h-4" /> {d.leechers.toLocaleString()} leechers</span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <a href={d.magnet} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full glass hover:bg-primary/20 text-sm font-medium transition-colors">
            <Magnet className="w-4 h-4" /> Magnet
          </a>
          <a href={d.torrent} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full gradient-primary text-primary-foreground text-sm font-semibold shadow-glow hover:scale-105 transition-transform">
