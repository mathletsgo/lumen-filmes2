...
import { getTVDetails } from "@/services/api/tvdb";

export const Route = createFileRoute("/favorites")({
  head: () => ({
    meta: [
      { title: "Favoritos — Lumen" },
      { name: "description", content: "Sua lista pessoal de filmes e séries favoritos no Lumen." },
    ],
  }),
  component: Favorites,
});
...
function Favorites() {
  ...
  const queries = useQueries({
    queries: items.map((item) => ({
      queryKey: ["tmdb", item.type, item.id],
      queryFn: () => (item.type === "tv" ? getTVDetails(item.id) : getMovieDetails(item.id)),
    ...
    <div className="pt-32 pb-16 px-4 sm:px-12 max-w-screen-2xl mx-auto">
      ...
      {items.length === 0 ? (
        <div className="mt-20 text-center">
          ...
          <p className="text-muted-foreground mt-2">Comece a favoritar filmes e séries para vê-los aqui.</p>
          <Link
            to="/"
        ...
        <div className="mt-10">
          <MovieGridSkeleton count={items.length} />
        </div>
      ) : (
