...
function Favorites() {
  ...
  const list = queries.map((q) => q.data).filter(Boolean) as NonNullable<(typeof queries)[number]["data"]>[];
  ...
  return (
    <div className="pt-32 pb-16 px-4 sm:px-12 max-w-screen-2xl mx-auto">
      ...
      <p className="text-muted-foreground mt-3">{ids.length} {ids.length === 1 ? "título salvo" : "títulos salvos"}.</p>
      ...
      {ids.length === 0 ? (
        <div className="mt-20 text-center">
          ...
          <Link to="/" className="inline-block mt-6 px-7 py-3 rounded-full gradient-primary text-primary-foreground font-semibold shadow-glow hover:scale-105 transition-transform">
            Explorar filmes
          </Link>
        </div>
      ) : isLoading ? (
        <div className="mt-10"><MovieGridSkeleton count={ids.length} /></div>
      ) : (
